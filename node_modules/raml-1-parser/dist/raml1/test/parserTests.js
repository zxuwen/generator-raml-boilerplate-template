...
describe('Parser regression tests', function () {
    ...
    it('Should correctly serialize multiple examples to JSON', function () {
        ...
        var topLevelApi = api.wrapperNode();
...
describe('Parse strings', function () {
    ...
    it('scalar should include both start/end quotes #parse-str1', function () {
        ...
        var resource = tools.collectionItem(elementsOfKind, 0);
...
describe('Optional template parameters tests', function () {
    ...
    it("Should report error on unspecified parameter, which is used after expansion #1.", function () {
        testErrors(util.data("parser/optionalTemplateParameters/api02.raml"), ["Value is not provided for parameter: 'param1'"]);
    });
    ...
    it("Should report error on unspecified parameter, which is used after expansion #2.", function () {
        testErrors(util.data("parser/optionalTemplateParameters/api04.raml"), ["Value is not provided for parameter: 'param1'"]);
    });
    ...
    it("Only methods are permitted to be optional in sense of templates expansion.", function () {
        testErrors(util.data("parser/illegalOptionalParameters/api01.raml"), ["Only method nodes can be optional"]);
    });
});
...
function testDump(apiPath, options) {
...
function testErrorsEnd(p) {
    var api = util.loadApi(p);
    var errors = util.validateNode(api);
    var issue = errors[0];
    assert.equal(issue.end < api.lowLevel().unit().contents().length, true);
}
function testErrors(p, expectedErrors, ignoreWarnings) {
    ...
    var errors = util.validateNode(api);
    if (ignoreWarnings) {
        errors = errors.filter(function (x) { return !x.isWarning; });
    }
    var testErrors;
    ...
    if (hasUnexpectedErr || errors.length != expectedErrors.length) {
        ...
        expectedErrors.forEach(function (expectedError) { return console.warn(expectedError); });
        var unitContents = api.lowLevel().unit().contents();
        console.warn("Actual errors:");
        errors.forEach(function (error) { return console.warn(error.message + " : " + unitContents.substr(error.start, error.end - error.start)); });
    }
...
function testErrorsByNumber(p, count, deviations) {
